...
startGame file = parser file >> printState Playing >>
                 iterateWhile ( (==) Playing ) playGame >> return ()

playGame :: Game GameConditions
playGame = doMove >>= updateTable >>= printState
    where doMove = iterateWhile (not . validMove) (liftIO $ hGetChar stdin) >>=
                   moveRobot . read . (flip (:) [])
...
moveRobotAux movement tbl m_from@(x,y) (m_to,c_to)
    | isEmpty c_to || isEarth c_to = do
        liftIO $ writeArray tbl m_from empty
        liftIO $ writeArray tbl m_to   robot
        putRobotPos m_to
        return True
    | isTrampoline c_to = do
        liftIO $ writeArray tbl m_from empty
        ...
        liftIO $ mapM_ (\pos -> writeArray tbl pos empty) (getSources jump_to ts [])
        return True
    | isLambda c_to = do
        liftIO $ writeArray tbl m_from empty
        liftIO $ writeArray tbl m_to   robot
    ...
    | movement == MRight && isRocky c_to = do
        ...
        if isEmpty temp
           then do
               liftIO $ writeArray tbl m_next rock
               liftIO $ writeArray tbl m_from empty
               liftIO $ writeArray tbl m_to   robot
               putRobotPos m_to
               return True
  ...
  where razeThemUp :: Game ()
        razeThemUp = do
            -- Raze them all! }:-] (sudo make me readable)
...
updateTable _ = do
  ...
  where incMovesUnderWaterIfNeeded :: Dimension -> Game ()
        incMovesUnderWaterIfNeeded (_,y) = do gs <- get
...
updateTableAux' new_t must_grow elems (x,y)
    ...
    | isRocky e && isLambda ed && isEmpty er && isEmpty erd
                               = do writeArray new_t (x,y) empty
                                    moveRocky (x+1,y-1) erdd
    | isBeard e && must_grow   = do mapM_ (\(it,i) ->
                                            if isEmpty it || isBeard it
                                              then writeArray new_t i beard
                                              else return ()) fringe
    | otherwise                = do ru <- recentlyUpdated (x,y)
                                    when (not ru) $ writeArray new_t (x,y) e
...
printTable (size_x,size_y) tbl waterLevel =
    mapM_ ppAll [size_y,size_y-1..1]
